...
var ProductCard = class extends HTMLElement {
  ...
  async _onSwatchChanged(event, target) {
    if (target.hasAttribute("data-variant-id")) {
      ...
      const quickBuy = this.querySelectorAll(".product-card__quick-buy");
      if(quickBuy){
        quickBuy.forEach( item => item.querySelector('input[name="id"]').value = target.getAttribute("data-variant-id"));
      }
    }
